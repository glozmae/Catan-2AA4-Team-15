...
The first computer player can use an OpenAI model as a small strategy advisor. The original Java rules still decide which moves are legal. The LLM only chooses one of those legal moves and gives a short reason.

```text
Java checks the rules -> legal move list -> LLM chooses a move number -> Java executes that legal move
```
...
The default model is `gpt-5.6-luna`. To use another compatible model for the current terminal session:
